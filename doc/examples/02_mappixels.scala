...
def run(sc: SparkContext): RasterRDD[Int] = {
    ...
    val mapPixelRaster: RasterRDD[Int] = raster.mapPixels(pixel => pixel + 10)
        
    // Force execution + small driver-side sanity check
    val t = mapPixelRaster.first
    val v = t.getPixelValue(t.x1, t.y1)
    println(s"[dataloading] pixelType=${t.pixelType}, sample=$v, partitions=${mapPixelRaster.getNumPartitions}")
